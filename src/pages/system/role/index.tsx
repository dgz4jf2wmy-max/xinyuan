...
import { getRolePage } from '../../../data/system/roleData';
...
export default function RoleManagement() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getRolePage(queryParams.pageNum, queryParams.pageSize, {
  ...
  const handleSave = () => {
    console.log('保存角色数据', currentRole);
      ...
      >
        <div className="space-y-4 py-2">
          <div className="flex items-center gap-4">
            <label className="w-20 text-right text-sm text-gray-700"><span className="text-red-500 mr-1">*</span>角色名称</label>
            <Input 
              className="flex-1" 
          ...
          <div className="flex items-center gap-4">
            <label className="w-20 text-right text-sm text-gray-700"><span className="text-red-500 mr-1">*</span>权限字符</label>
            <Input 
              className="flex-1" 
              value={currentRole.roleCode || ''} 
              onChange={e => setCurrentRole({...currentRole, roleCode: e.target.value})}
              placeholder="请输入权限字符" 
            />
          </div>
          <div className="flex items-start gap-4">
            <label className="w-20 text-right text-sm text-gray-700 mt-2">描述</label>
            <textarea 
              className="flex-1 min-h-[80px] rounded-md border border-gray-300 p-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500"
