...
export default function MobileLabelingTaskPage() {
  ...
  const filteredTasks = mockLabelingTasks.filter(
    (task) =>
          ...
          <MobileListItem key={task.id}>
            ...
            <div className="grid grid-cols-1 gap-y-2 mt-3 bg-slate-50 p-2.5 rounded text-[12px]">
              <div className="flex justify-between items-start gap-2">
                <span className="text-slate-500 shrink-0">牌号</span>
                <span className="text-slate-700 text-right">{task.brandCode}</span>
              </div>
