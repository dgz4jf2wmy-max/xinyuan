...
export default function ForemanLogs() {
          ...
          <TableBody>
            {mockForemanLogs.map((log) => (
              <TableRow key={log.id} className="hover:bg-[#f5f7fa] transition-colors">
                ...
                <TableCell className="text-center border-r border-[#ebeef5] last:border-r-0 max-w-[300px] truncate" title={log.logName}>{log.logName}</TableCell>
                ...
                <TableCell className="text-center border-r border-[#ebeef5] last:border-r-0">
                  <Button variant="link" size="sm" className="h-auto p-0 text-[#409eff] font-normal hover:text-[#66b1ff]" onClick={() => navigate(`/production/handover/log/foreman-detail/${log.id}`)}>查看</Button>
                </TableCell>
              </TableRow>
            ))}
            {mockForemanLogs.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="h-32 text-center text-[#c0c4cc]">
